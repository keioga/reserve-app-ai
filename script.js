...
async function syncData() {
    ...
    try {
        ...
        reservations = data.reservations.map(r => ({
            ...
            pcIndex: (r.pcIndex === "" || r.pcIndex === undefined || r.pcIndex === null) ? null : Number(r.pcIndex)
        }));
        ...
        reservations = JSON.parse(localStorage.getItem('sch_v5_res')) || [];
        students = JSON.parse(localStorage.getItem('sch_v5_students')) || [];
    } finally {
...
function renderSlots(date) {
    ...
    TIMES.forEach(t => {
        ...
        const count = reservations.filter(r => r.date === date && r.time === tNorm).length;
        const remain = STUDENT_CAPACITY - count;
        const booked = reservations.some(r => r.studentId === currentStudent.id && r.date === date && r.time === tNorm);
        ...
        let isWarn = !isSpecialBlock && !booked && remain > 0 && remain <= 2;
        const div = document.createElement('div');
        div.className = `slot-card ${isBlocked && !isWarn ? 'full' : (isWarn ? 'warn-block' : '')}`;
        let txt = isSpecialBlock ? "－" : (booked ? "済" : (isWarn ? "▲" : (remain <= 0 ? "×" : "〇")));
