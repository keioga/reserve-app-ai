...
async function registerStudent() {
    ...
    const studentData = { 
        ...
        carryOver: document.getElementById('s_carry').value, // 繰越
        status: document.getElementById('s_status').value // ★これを追加
    };
...
async function makeBooking() {
    ...
    try {
        ...
        const limit = Number(currentStudent.limit);
        const carry = Number(currentStudent.carryOver || 0);
        const totalCap = limit + carry; // 今月の総枠（基本＋繰越）

        const monthRes = reservations.filter(r => String(r.studentId) === String(currentStudent.id) && r.date.startsWith(month));
        const futureResCount = monthRes.filter(r => r.date >= todayStr).length;

        // 【制約1：総受講枠のチェック】
        if (monthRes.length + selected.length > totalCap) {
            throw new Error(`今月の総受講可能枠（${totalCap}回）を超えています。\n(基本:${limit} + 繰越:${carry})\n現在の合計:${monthRes.length}回`);
        }
        // 【制約2：同時予約数（保持できる未来の予約数）のチェック】
        if (futureResCount + selected.length > limit) {
            throw new Error(`一度に保持できる予約は最大${limit}回までです。\n受講を消化してから追加予約してください。`);
        }
...
function renderMonthlyList() {
    ...
    let html = `<table class="monthly-list-table"><thead><tr><th>受講生名</th><th>枠</th><th>越</th><th>残</th>`;
    for(let i=1; i<=20; i++) { html += `<th>${i}</th>`; }
    html += `</tr></thead><tbody>`;

    // --- 修正点1：表示対象の受講生リストを動的に作成する ---
    const monthResAll = reservations.filter(r => r.date.startsWith(prefix));
    
    // 現在の名簿（students）をコピー
    let displayStudents = [...students];

    // その月の予約データには存在するが、現在の名簿にはいない人（退会者など）を探して追加
    monthResAll.forEach(r => {
        const sid = String(r.studentId).trim();
        if (sid !== "EXAM" && !displayStudents.some(s => String(s.id) === sid)) {
        ...
        // if (!displayStudents.some(s => String(s.id) === String(r.studentId))) {
        //     displayStudents.push({
        //         id: r.studentId,
        //         name: r.name + "(退)", // 退会者であることが分かるように表記
        //         limit: "---",
        //         carryOver: 0,
        //         isWithdrawn: true
        //     });
    ...
    // --- 修正点2：作成した displayStudents を使ってループを回す ---
    displayStudents.forEach(s => {
        const myMonthRes = monthResAll.filter(r => String(r.studentId) === String(s.id))
                           .sort((a,b) => a.date.localeCompare(b.date) || a.time.localeCompare(b.time));

        // 予約が1件もない退会者は表示しない（名簿の現在者、または予約がある人のみ表示）
        if (s.isWithdrawn && myMonthRes.length === 0) return;

        const carry = Number(s.carryOver || 0);
        // 退会者の場合は「残」を表示せず受講数のみ分かるように調整（必要に応じて）
        const remaining = s.isWithdrawn ? "済" : (Number(s.limit) + carry - myMonthRes.length);

        html += `<tr><td style="text-align:left;">${s.name}</td><td>${s.limit}</td><td>${carry}</td><td>${remaining}</td>`;
    ...
    //     const carry = Number(s.carryOver || 0); // 繰越
    //     const remaining = Number(s.limit) + carry - myMonthRes.length; // ★指示通りの計算式
...
function handleStudentEditSelect() {
    ...
    if (s) {
        ...
        document.getElementById('s_carry').value = s.carryOver || 0;
...
function openInitModal() {
    const sel = document.getElementById('initTargetMonth');
    sel.innerHTML = '';
    const now = new Date();
    // 今月、先月、先々月を選択肢に出す
    for (let i = 0; i < 3; i++) {
        ...
        opt.value = val;
        opt.innerText = d.getFullYear() + "年" + (d.getMonth() + 1) + "月度";
        sel.appendChild(opt);
    }
...
// 初期化処理の実行
async function executeMonthlyInitialization() {
    const targetMonth = document.getElementById('initTargetMonth').value;
    const msg = `${targetMonth} の更新処理を実行します。\n\n` +
                "・受講実績を「履歴」シートへバックアップします\n" +
                "・未受講分を「繰越」へ反映します\n" +
                "・「退会」設定の生徒を名簿から削除します\n" +
                "・前々月以前の予約データを削除して高速化します\n\n" +
                "よろしいですか？";
    ...
    try {
        await fetch(GAS_URL, {
            method: 'POST',
            mode: 'no-cors',
            body: JSON.stringify({ type: "batchInitialization", targetMonth: targetMonth })
        });
        
        alert("更新処理が完了しました。");
        closeInitModal();
        await syncData(); // 繰越反映後のデータを読み込み

        if (confirm("続けて印刷用画面（前月分）を表示しますか？")) {
            switchAdminSubTab('month');
            const dParts = targetMonth.split('-');
            adminListMonth = new Date(dParts[0], dParts[1] - 1, 1);
            renderMonthlyList();
        }
    } catch (e) {
        alert("エラーが発生しました。");
